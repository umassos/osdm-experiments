...
T = 48           # 24 hours in 15-minute intervals
c_delivery = 0.2
eps_delivery = 0.05
epochs = 1000
...
def precompute_opt_costs_flex(price_instances, base_instances, flex_instances, Delta_instances,
    ...
    """
    # compute one hash for all instances to identify the configuration and save on redundant computations
    # price instances are a list of lists, which are not hashable, so convert to tuples
    instance_hash = hash((tuple(tuple(seq) for seq in price_instances),
                          tuple(tuple(seq) for seq in base_instances),
                          tuple(tuple(seq) for seq in flex_instances),
                          tuple(tuple(seq) for seq in Delta_instances)))
    ...
    if os.path.exists(f"opt_sols/opt_costs_flex_{instance_hash}.pkl"):
        with open(f"opt_sols/opt_costs_flex_{instance_hash}.pkl", "rb") as f:
            opt_costs, total_demands_saved = pickle.load(f)
        print(f"Loaded precomputed OPT costs for flexible demand from opt_sols/opt_costs_flex_{instance_hash}.pkl")
    ...
    with open(f"opt_sols/opt_costs_flex_{instance_hash}.pkl", "wb") as f:
...
# Prefer using OPT purchases: set y to constant at (avg price OPT "accepted" + gamma),
# where accepted means x_opt[t] > 0. If OPT isn't available, fall back to
# (global average training price + gamma). Always pin the last value to (p_min + 2*gamma).

def compute_opt_accepted_avg_price(opt_costs, total_demands):
    instance_avg_prices = []
    for opt_cost, total_demand in zip(opt_costs, total_demands):
        if opt_cost is not None and total_demand > 1e-6:
            avg_price = opt_cost / total_demand
            instance_avg_prices.append(avg_price)
    if instance_avg_prices:
        return sum(instance_avg_prices) / len(instance_avg_prices)
    return None

# Compute OPT-accepted average price if possible
print("Computing OPT-accepted average price...")
opt_accepted_avg = compute_opt_accepted_avg_price(opt_costs_all, total_demands_all)

# Fallback global average price across training instances
if price_all:
    total_sum = 0.0
    total_count = 0
    for seq in price_all:
        for v in seq:
            total_sum += float(v)
            total_count += 1
    p_avg = total_sum / total_count if total_count > 0 else float(p_max)
else:
    p_avg = float(p_max)

y0_val = (opt_accepted_avg) if (opt_accepted_avg is not None) else (p_avg)
if opt_accepted_avg is not None:
    print(f"OPT-accepted average price: {opt_accepted_avg:.4f}")
last_val = float(p_min) + 2.0 * gamma
y_init = [y0_val for _ in range(K)]
y_init_delivery = [y0_val * (c_delivery + eps_delivery) for _ in range(K)]

# Ensure non-increasing: y[i] >= y[i+1]
for i in range(1, K):
    if y_init[i] > y_init[i-1]:
        y_init[i] = y_init[i-1]

# Set the last value exactly as requested (may break non-increasing at the tail by design)
if K > 0:
    y_init[-1] = last_val
    y_init_delivery[-1] = last_val * (c_delivery + eps_delivery)

y = torch.nn.Parameter(torch.tensor(y_init, dtype=torch.float32))
y_flex_p = torch.nn.Parameter(torch.tensor(y_init, dtype=torch.float32))  # flexible purchase threshold
# Delivery threshold can be initialized to same scale
y_flex_d = torch.nn.Parameter(torch.tensor(y_init_delivery, dtype=torch.float32))
...
# # Create learnable tensors from random init
# y = torch.nn.Parameter(torch.tensor(y_random, dtype=torch.float32))
# y_flex_p = torch.nn.Parameter(torch.tensor(y_flex_p_random, dtype=torch.float32))
# y_flex_d = torch.nn.Parameter(torch.tensor(y_flex_d_random, dtype=torch.float32))
...
def torch_objective(p_seq, x_seq, z_seq, gamma, delta, c, eps):
    ...
    for t in range(Tn):
        s_t = torch.maximum(s_prev + x_seq[t] - z_seq[t], torch.tensor(0.0))
        s.append(s_t)
        s_prev = s_t
    s_torch = torch.stack(s)  # s[0..T-1] corresponds to s_1..s_T in numpy version

    # Costs
    cost_purchasing = (p_seq * z_seq).sum()
    ...
    s_prev_seq = torch.cat([torch.tensor([0.0]), s_torch[:-1]])
...
# Note: We no longer train on competitive ratio, so we skip precomputing OPT costs for the batch.
...
try: 
    for epoch in range(epochs):
        # if the loss has not changed for 100 epochs, reinitialize y to random thresholds
        if epoch > 100 + reinitialized:
            # look at the losses in the last 100 epochs, if loss has not improved, reinitialize
            cur_best_epoch = best_snapshot["batch"]
            if epoch - cur_best_epoch > 100:
                print(f"Epoch {epoch}: Loss has not improved for 100 epochs, reinitializing y to random values.")
                ...
                with torch.no_grad():
                    ...
                    min_price = min(price_seq)
                    y_random = torch.rand(K) * (float(price_max) - float(min_price)) + float(min_price)
                    y_random, _ = torch.sort(y_random, descending=True)
                    y_random[-1] = float(p_min) + 2.0 * float(gamma)
                    y.copy_(y_random)

                    y_flex_p_random = torch.rand(K) * (float(price_max) - float(min_price)) + float(min_price)
                    y_flex_p_random, _ = torch.sort(y_flex_p_random, descending=True)
                    y_flex_p_random[-1] = float(min_price) + 2.0 * float(gamma)
                    y_flex_p.copy_(y_flex_p_random)

                    y_flex_d_random = torch.rand(K) * (float(price_max) * (c_delivery + eps_delivery) - float(min_price) * (c_delivery + eps_delivery)) + float(min_price) * (c_delivery + eps_delivery)
                    y_flex_d_random, _ = torch.sort(y_flex_d_random, descending=True)
                    y_flex_d_random[-1] = float(min_price) * (c_delivery + eps_delivery) + 2.0 * float(delta)
