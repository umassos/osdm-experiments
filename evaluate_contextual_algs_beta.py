...
parser.add_argument('--rho_factor', type=float, default=5.0, help='Rho factor for robust projection (default: 5.0)')
...
alpha = get_alpha(1, 200, gamma, delta, c_delivery, eps_delivery, T)
y_base_analytical = torch.tensor([base_threshold((i + 0.5) / K, 1, 200, gamma, delta, c_delivery, eps_delivery, T, alpha) for i in range(K+1)])
y_flex_p_analytical = torch.tensor([flex_purchase_threshold((i + 0.5) / K, 1, 200, gamma, delta, c_delivery, eps_delivery, T, alpha) for i in range(K+1)])
y_flex_d_analytical = torch.tensor([flex_delivery_threshold((i + 0.5) / K, 1, 200, gamma, delta, c_delivery, eps_delivery, T, alpha) for i in range(K+1)])
...
def forward_pald(price_seq, time_seq, month_seq, forecast_seq, base_seq, flex_seq, Delta_seq, model, p_min, p_max):
    ...
    if args.rho_factor < 1.0:
        y_vec_t, _, _ = y_base_analytical, y_flex_p_analytical, y_flex_d_analytical
    ...
    for t in range(T):
        ...
        if f_arrival > 0:
            ...
            if args.rho_factor < 1.0:
                _, y_vec_p, y_vec_d = y_base_analytical, y_flex_p_analytical, y_flex_d_analytical
...
def main():
    global y_base_analytical, y_flex_p_analytical, y_flex_d_analytical
    max_month = 12
    global_p_min = 1
    global_p_max = 100
    ...
    for month in month_range:
        ...
        global_p_min = min(global_p_min, p_min)
        global_p_max = max(global_p_max, p_max)
    ...
    alpha = float(get_alpha(float(global_p_min), float(global_p_max), c_delivery, eps_delivery, T, gamma, delta))
    y_base_analytical = torch.tensor([base_threshold((i + 0.5) / K, float(global_p_min), float(global_p_max), gamma, delta, c_delivery, eps_delivery, T, alpha) for i in range(K+1)])
    y_flex_p_analytical = torch.tensor([flex_purchase_threshold((i + 0.5) / K, float(global_p_min), float(global_p_max), gamma, delta, c_delivery, eps_delivery, T, alpha) for i in range(K+1)])
    y_flex_d_analytical = torch.tensor([flex_delivery_threshold((i + 0.5) / K, float(global_p_min), float(global_p_max), gamma, delta, c_delivery, eps_delivery, T, alpha) for i in range(K+1)])

    print(f"Computed alpha for analytical thresholds: {alpha}")
    beta = rho_factor*alpha
    print(f"Using beta = {beta} for robust projection")
    ...
    try:
        ...
        model = ThresholdPredictor(input_dim=11, K=K+1, hidden_dims=(64, 64), beta=beta, p_min=float(global_p_min), p_max=float(global_p_max), use_robust_projection=True)
        if rho_factor < 1.0:
            model.use_robust_projection = False
            model.beta = 0.0
    ...
    for month in month_range:
        ...
        price_all, times_all, months_all, forecast_all, base_all, flex_all, Delta_all, p_min, p_max = month_data[month - 1 - month_min]
    ...
    output_file = f'eval_results/{prefix}_rho{rho_factor}_{args.trace}_T{args.T}_gamma{args.gamma}_delta{delta}_c{args.c_delivery}_eps{args.eps_delivery}_prop{args.proportion_base}_scale{scale_factor}.pkl'
