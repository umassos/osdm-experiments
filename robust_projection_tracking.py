...
def project_y_robust(y_tensor: torch.Tensor, K: int, p_min: float, p_max: float,
                     eta: float, delta: float, c: float, eps: float, T: int,
                     beta: float = 6.0) -> list:
    ...
    constraints += [y_var[K-1] <= float(p_min) + 2.0 * float(eta)]
    ...
    for j in range(0, K+1):
        ...
        rhs = beta * (psi_w - 2 * eta + eps * p_max + (2 * delta) / T)
        constraints += [lhs <= rhs]
...
def project_y_flex_robust(y_phi_tensor: torch.Tensor,
    ...
    for j in range(0, K + 1):
        ...
        for l in range(0, K + 1):
            ...
            rhs = beta * (fac * (phi_nodes[j] + psi_nodes[l] - 2.0 * kappa) + (2.0 * delta) / T)
            cons += [lhs <= rhs]
