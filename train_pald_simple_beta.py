...
parser.add_argument('--epochs', type=int, default=220, help='Number of training epochs (default: 220)')
...
trace = args.trace
# get rho factor from command line
rho = args.rho_factor
    ...
    for epoch in range(epochs):
        ...
        for batch_idx in range(num_batches):
            ...
            for idx, (price_seq, base_seq, flex_seq, Delta_seq) in enumerate(zip(price_batch, base_batch, flex_batch, Delta_batch)):
                ...
                base_drivers.append({"id": 0, "b": adjusted_S, "w": 0.0, "prev_decision": 0.0})
...
with torch.no_grad():
    if best_snapshot["y"] is not None:
        ...
        import pickle
        best_outfile = f"best_thresholds/best_thresholds_rhofactor_{rho}_{trace}_{month}_{batch_size}_{run_tag}.pkl"
...
def forward_pald(price_seq, base_seq, flex_seq, Delta_seq):
    ...
    storage_state = 0.0
    x_prev_global = torch.tensor(0.0)
    # adjusted_S = 40.0 / args.scale_factor
    adjusted_S = min(sum(base_seq), 1.0)
