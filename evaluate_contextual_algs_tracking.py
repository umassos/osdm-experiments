...
parser.add_argument("--eta", type=float, default=10.0, help="Eta tracking cost parameter (default: 10.0)")
...
def forward_pald(price_seq, time_seq, month_seq, forecast_seq, base_seq, flex_seq, Delta_seq, tracking_seq, model, p_min, p_max):
    ...
    feats = build_driver_features(
        t_idx=0, T=T, price_seq=price_seq, time_seq=time_seq, month_seq=month_seq, forecast_seq=forecast_seq, tracking_seq=tracking_seq,
        storage_state=0.0,
    ...
    for t in range(T):
        ...
        if f_arrival > 0:
            feats = build_driver_features(
                t_idx=t, T=T, price_seq=price_seq, time_seq=time_seq, month_seq=month_seq, forecast_seq=forecast_seq, tracking_seq=tracking_seq,
        ...
        for drv in base_drivers:
            ...
            # get sum of current demands (all drivers)
            total_current_demand = sum(d["b"] for d in base_drivers) + sum(f["f"] for f in flex_drivers)
            # share positive excess proportional to previous physical contribution
            ...
            eta_t = torch.tensor(eta, dtype=torch.float32).reshape(-1)
            p_t_t = torch.tensor([p_t_val], dtype=torch.float32)
            pseudo_a_t_val = a_t_val * b_i / total_current_demand
            a_t_t = torch.tensor([pseudo_a_t_val], dtype=torch.float32)

            cur_frac_decision = _safe_layer_call(
                pald_base_layer, y_vec_t, (a_t_t, w_eff, p_t_t, eta_t, w_hinge_t, c1_t)
            )
        ...
        for fd in flex_drivers:
            ...
            if t == max(0, int(fd["delta"])-1):
                ...
                eta_t = torch.tensor(eta, dtype=torch.float32).reshape(-1)
                p_t_t = torch.tensor([p_t_val], dtype=torch.float32)
                pseudo_a_t_val = a_t_val * f_i / total_current_demand
                a_t_t = torch.tensor([pseudo_a_t_val], dtype=torch.float32)


                cur_frac_x = _safe_layer_call(
                    pald_flex_purchase_layer, y_vec_t, (a_t_t, w_eff, p_t_t, eta_t, w_hinge_t, c1_t)
                )
...
def evaluate_many(price_all, times_all, months_all, forecast_all, base_all, flex_all, Delta_all, tracking_target_all, p_min, p_max,
    ...
    for idx in range(num_instances):
        ...
        tracking_seq = tracking_target_all[idx]
        b_seq = base_all[idx]
        f_seq = flex_all[idx]
        D_seq = Delta_all[idx]

        # PALD-Fast
        pald_x, pald_z, storage_state = forward_pald(p_seq, times_seq, month_seq, forecast_seq, b_seq, f_seq, D_seq, tracking_seq, model, p_min, p_max)
        last_price = p_seq[-1]
        pald_cost = np_objective_function(T, p_seq, eta/5, delta, c_delivery, eps_delivery, pald_x, pald_z, tracking_seq) - storage_state * last_price - 2*eta*storage_state
        
        # PAAD
        # paad_res = pi.paad_algorithm(T, p_seq, eta, delta,
        #                              c_delivery, eps_delivery,
        #                              p_min, p_max, S, b_seq, f_seq, D_seq, tracking_seq)
        # paad_x = paad_res["x"]
        # paad_z = paad_res["z"]
        # paad_cost = np_objective_function(T, p_seq, eta, delta, c_delivery, eps_delivery, paad_x, paad_z, tracking_seq)
        ...
        if opt_recompute:
            try:
                status, results = opt_sol.optimal_tracking_solution(
                    T, p_seq, eta, delta, c_delivery, eps_delivery, S, b_seq, f_seq, D_seq, tracking_seq
                )
                if status == "Optimal" and results is not None:
                    oc = np_objective_function(T, p_seq, eta, delta, c_delivery, eps_delivery,
                                                results["x"], results["z"], tracking_seq)
                    opt_costs.append(oc)
...
def main():
    ...
    for month in range(1, max_month + 1):
        ...
        month_data.append((price_all, times_all, months_all, forecast_all, base_all, flex_all, Delta_all, tracking_target_all, p_min, p_max))
    ...
    try:
        ...
        matching_files = [f for f in files if args.trace in f and "tracking" in f and f.endswith('.pt')]
        ...
        model = ThresholdPredictor(input_dim=12, K=K+1, hidden_dims=(64, 64), beta=100, p_min=float(p_min), p_max=float(p_max), use_robust_projection=True)
    ...
    for month in range(1, max_month + 1):
        ...
        price_all, times_all, months_all, forecast_all, base_all, flex_all, Delta_all, tracking_target_all, p_min, p_max = month_data[month - 1]
        rows_month = evaluate_many(price_all, times_all, months_all, forecast_all, base_all, flex_all, Delta_all, tracking_target_all, p_min, p_max, model, args, month=month)
    ...
    output_file = f'eval_results/{prefix}_{args.trace}_T{args.T}_eta{args.eta}_delta{args.delta}_c{args.c_delivery}_eps{args.eps_delivery}_prop{proportion_base}_scale{scale_factor}.pkl'
