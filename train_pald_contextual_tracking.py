...
from pald_implementation_tracking import (
...
feature_dim = 12  # handcrafted features length (see build_driver_features)
...
# ---------------------------------------
def precompute_opt_costs_tracking(price_instances, base_instances, flex_instances, Delta_instances, tracking_target_all, T, eta, delta, c, eps, S):
    """
...
opt_costs_all, opt_avgp_all = precompute_opt_costs_tracking(
    price_all, base_all, flex_all, Delta_all, tracking_target_all, T, eta, delta, c_delivery, eps_delivery, S
)
...
def _sample_gate_stats(model: ThresholdPredictor, sample_n: int,
    ...
    with torch.no_grad():
        for i in idxs:
            feats = build_driver_features(0, T, price_all[i], times_all[i], months_all[i], forecast_all[i], tracking_target_all[i], 0.0, "base", S, 0, float(p_min), float(p_max)).to(device)
            try:
...
try: 
    ...
    # Snapshot params to measure actual parameter movement during this epoch
        ...
        for batch_idx in range(num_batches):
            ...
            tracking_target_batch = tracking_target_all[start:end]
            batch_total_loss = torch.tensor(0.0)
            warmup_loss_sum = torch.tensor(0.0, requires_grad=True)

            batch_crs = []  # competitive ratios for this batch
            # Iterate instances in this batch
            for idx, (price_seq, time_seq, month_seq, forecast_seq, base_seq, flex_seq, Delta_seq, tracking_seq) in enumerate(zip(price_batch, time_batch, month_batch, forecast_batch, base_batch, flex_batch, Delta_batch, tracking_target_batch)):
                ...
                a_torch = torch.tensor([float(v) for v in tracking_seq], dtype=torch.float32, requires_grad=True)
                ...
                if not use_cost_loss:
                    ...
                    inst_loss = torch.tensor(0.0, requires_grad=True)
            ...
            if warmup_active:
                ...
                loss = (warmup_loss_sum / max(1, batch_size)) * torch.tensor(args.warmup_lambda, dtype=torch.float32).reshape(-1)
            else:
                ...
                if len(price_batch) > 0:
                    feats_s = build_driver_features(0, T, price_batch[0], time_batch[0], month_batch[0], forecast_batch[0], tracking_target_batch[0], 0.0, "base", S, 0, float(p_min), float(p_max))
...
if best_snapshot["model_state"] is not None:
    try:
        ...
        model_path = f"best_models/pald_tracking_model_{trace}_{month}_{batch_size}_{run_tag}.pt"
        ...
        if best_snapshot["model_state"] is not None:
            ...
            try:
                ...
                if sample_idx is not None:
                    ...
                    tracking_seq = tracking_target_all[sample_idx]
