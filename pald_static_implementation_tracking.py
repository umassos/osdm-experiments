...
def make_pald_base_layer(K, eta):
    ...
    track_cost = eta * cp.abs(x_total - a_t)
    phi_cost = y_vec @ x_parts
    obj = cp.Minimize(hit_cost + track_cost - phi_cost + ridge)
    prob = cp.Problem(obj, constraints)
    return CvxpyLayer(prob,
                      parameters=[a_t, w_prev, p_t, y_vec, caps],
                      variables=[x_total])

def make_pald_flex_purchase_layer(K, eta):
    x_parts = cp.Variable(K, nonneg=True)
    x_total = cp.Variable(nonneg=True)
