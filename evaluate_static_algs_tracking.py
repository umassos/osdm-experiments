...
parser.add_argument("--gamma", type=float, default=0.0, help="Gamma switching cost parameter (default: 10.0)")
...
parser.add_argument("--eta", type=float, default=10.0, help="Tracking cost parameter eta (default: 0.0, no tracking cost)")
...
def forward_pald_fast_reuse(models, price_seq, base_seq, flex_seq, Delta_seq, a_seq,
    ...
    base_drivers = [{"id": 0, "b": adjusted_S, "w": 0.0, "prev_decision": 0.0}]
    flex_drivers = []
    ...
    for t in range(T):
        ...
        for drv in base_drivers:
            ...
            pseudo_a_t_val = a_t_val * b_i / denom if denom > 0 else 0.0
            ...
            if (1.0 - w_eff) <= 1e-9 or sum(caps_list) <= 1e-12:
                ...
                cur_frac = _solve_base_cvx(base_m, x_prev_clamped, pseudo_a_t_val, w_eff, p_t_val, y_base, caps_list)
        ...
        for fd in flex_drivers:
            ...
            pseudo_a_t_val = a_t_val * f_i / denom if denom > 0 else 0.0
            ...
            if (1.0 - w_eff) <= 1e-9 or sum(caps_list) <= 1e-12:
                ...
                x_prev_clamped = max(0.0, min(1.0 - w_eff, float(pseudo_prev_frac)))
                cur_frac = _solve_flex_purchase_cvx(flex_p_m, x_prev_clamped, pseudo_a_t_val, w_eff, p_t_val, y_flex_p, caps_list)
    ...
    for month in range(1, max_month + 1):
        ...
        if args.analytical:
            ...
            try:
                ...
                if args.eta > 0.0:
                    candidate = f"TRACKING_thresholds_scalefactor_{scale_factor}_{args.trace}_{month}_{args.num_instances}"
